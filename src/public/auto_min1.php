...
$sql00="SELECT COUNT(*) FROM auto WHERE j='$j' "; 
...
// Выбираем $num сообщений начиная с номера $start
			...
			if ($f==1) { $sql="SELECT * FROM auto WHERE j='$j'   ORDER BY $order LIMIT $start, $num";
		$result=mysqli_query($link,$sql);}
		   else { $sql="SELECT * FROM auto WHERE j='$j'  ORDER BY $order DESC LIMIT $start, $num";
		$result=mysqli_query($link,$sql);}	
