...
$result00=mysqli_query($link, "SELECT COUNT(*) FROM moto WHERE jl=$jl[0]  $bb2");
...
// Выбираем $num сообщений начиная с номера $start
	?>
			...
			if ($f==1) { 
				$result4=mysqli_query($link, "SELECT * FROM moto WHERE jl=$jl[0] $bb  ORDER BY $order LIMIT $start, $num");}
		else { 
			$result4=mysqli_query($link, "SELECT * FROM moto WHERE jl=$jl[0] $bb  ORDER BY $order DESC LIMIT $start, $num");}	
